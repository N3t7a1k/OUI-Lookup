...
class OUIDataManager:
    ...
    def load_from_cache(self):
        if self.cache_path.exists():
            try:
                ...
                return (datetime.now() - mtime).total_seconds() < 43200
            except Exception:
                pass
        return False
    ...
    def get_db_info(self):
        if not self.cache_path.exists():
            return {"ready": False, "updated_at": None, "next_update_at": None, "record_count": 0}
        mtime = datetime.fromtimestamp(self.cache_path.stat().st_mtime)
        next_update = datetime.fromtimestamp(self.cache_path.stat().st_mtime + 43200)
        return {
            "ready": self.df is not None,
    ...
    def search(self, query: str, page: int, limit: int):
        ...
        if is_mac_search:
            ...
            if len(results) == 0:
                try:
                    first_byte = int(clean_query[:2], 16)
                    ...
                    if is_multicast:
                        fake_result = {
                            "Assignment": query.upper(),
                            "Organization Name": "Multicast Address (Reserved)",
                            "Organization Address": "N/A - Multicast Group"
                        }
                    elif is_local:
                        fake_result = {
                            "Assignment": query.upper(),
                            "Organization Name": "Locally Administered Address (Randomized/Private)",
                            "Organization Address": "N/A - Random MAC"
                        }

                    if fake_result:
                        results = pd.DataFrame([fake_result])
...
def get_status():
    return jsonify(manager.get_db_info()), 200
